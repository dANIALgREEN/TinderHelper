...
async def gpt_dialog(update, context):
    ...
    my_message = await send_text(update, context, "Консерва набирает текст...")
...
async def date_dialog(update, context):
    text = update.message.text
    my_message = await send_text(update, context, "Партнёр набирает текст...")
...
async def date_button(update, context):
    ...
    await send_text(update, context, "Отличный выбор, можешь начинать диалог!")
...
async def message_button(update, context):
    ...
    prompt = load_prompt(query)
    user_chat_history = "\n\n".join(dialog.list)
    my_message = await send_text(update, context, "Консерва думает...")
...
async def opener_dialog(update, context):
    ...
    if dialog.count == 1:
        ...
        dialog.user["age"] = text
        await send_text(update, context, "Оцените её от 1 до 10:")
    elif dialog.count == 3:
...
app = ApplicationBuilder().token("7983982693:AAE1RaXuXiIrwJdOgdtbiD8rSrbYbfHb_XI").build()
